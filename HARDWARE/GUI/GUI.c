...
void LCD_BufferFill(uint16_t *buffer, uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color)
{
    uint16_t i, j;
...
/*******************************************************************
 * @name       :void LCD_BufferPresent(uint16_t *buffer)
 * @date       :2025-03-10
 * @function   :present the buffer to the screen
 * @parameters :None
 * @retvalue   :None
 ********************************************************************/
void LCD_BufferPresent(uint16_t *buffer)
{
    uint16_t i, j;
    LCD_SetWindows(0, 0, lcddev.width - 1, lcddev.height - 1); // 设置显示窗口
    for (i = 0; i < lcddev.height; i++)
    {
        for (j = 0; j < lcddev.width; j++)
        {
            Lcd_WriteData_16Bit(buffer[i * lcddev.width + j]); // 写入数据
        }
    }
}
...
void Gui_Drawbmp16(uint16_t x, uint16_t y, const unsigned char *p) // 显示40*40 QQ图片
{
    ...
    LCD_SetWindows(x, y, x + 40 - 1, y + 40 - 1); // 窗口设置，设置显示区域为从(x, y)到(x+39, y+39)
    for (i = 0; i < 40 * 40; i++)
    {
        picL = *(p + i * 2);                   // 数据低位在前，从指针p开始，每次取两个字节，第一个字节为低位
        picH = *(p + i * 2 + 1);               // 第二个字节为高位
        Lcd_WriteData_16Bit(picH << 8 | picL); // 将高位和低位组合成16位颜色数据，并写入LCD
    }
    LCD_SetWindows(0, 0, lcddev.width - 1, lcddev.height - 1); // 恢复显示窗口为全屏，即从(0, 0)到(lcddev.width-1, lcddev.height-1)
}

void Gui_Drawbmp(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const unsigned char *p) // 显示任意大小的图片
{
    uint64_t totalSize = width * height * sizeof(uint16_t); // 计算总数据长度
    uint64_t offset = 0;                                    // 数据偏移量
    uint32_t chunkSize = 38400;                             // 每次传输的数据长度
    LCD_SetWindows(x, y, x + width - 1, y + height - 1);    // 窗口设置，设置显示区域为从(x, y)到(x+width-1, y+height-1)
    LCD_CS_CLR;
    LCD_RS_SET;
    while (offset < totalSize)
    {
        uint32_t currentSize = (totalSize - offset) > chunkSize ? chunkSize : (totalSize - offset); // 计算当前批次的数据长度
        HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)(p + offset), currentSize);                         // 发送数据
        // 等待DMA传输完成
        while (HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY)
        {
        }
        offset += currentSize; // 更新数据偏移量
    }
    LCD_CS_SET;
...
void LCD_BufferDrawPoint(uint16_t *buffer, uint16_t x, uint16_t y)
{
    if (x < lcddev.width && y < lcddev.height)
    {
        buffer[y * lcddev.width + x] = POINT_COLOR;
    }
}

void LCD_BufferDrawLine(uint16_t *buffer, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    uint16_t t;
    int xerr = 0, yerr = 0, delta_x, delta_y, distance;
    int incx, incy, uRow, uCol;
    ...
    for (t = 0; t <= distance + 1; t++) // 画线输出
    {
        LCD_BufferDrawPoint(buffer, uRow, uCol); // 画点
        xerr += delta_x;
        yerr += delta_y;
        if (xerr > distance)
        {
...
void LCD_BufferDrawRectangle(uint16_t *buffer, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    LCD_BufferDrawLine(buffer, x1, y1, x2, y2);
    LCD_BufferDrawLine(buffer, x1, y1, x1, y2);
    LCD_BufferDrawLine(buffer, x1, y2, x2, y2);
    LCD_BufferDrawLine(buffer, x2, y1, x2, y2);
}

void Draw_BufferCircle(uint16_t *buffer, uint16_t x0, uint16_t y0, uint16_t fc, uint8_t r)
{
}

void Draw_BufferTriangel(uint16_t *buffer, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    LCD_BufferDrawLine(buffer, x0, y0, x1, y1);
    LCD_BufferDrawLine(buffer, x1, y1, x2, y2);
    LCD_BufferDrawLine(buffer, x2, y2, x0, y0);
}

void Fill_BufferTriangel(uint16_t *buffer, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    uint16_t a, b, y, last;
    int dx01, dy01, dx02, dy02, dx12, dy12;
    long sa = 0;
    ...
    if (y0 == y2)
    {
        ...
        LCD_BufferFill(buffer, a, y0, b, y0, POINT_COLOR);
        return;
    }
    dx01 = x1 - x0;
    dy01 = y1 - y0;
    ...
    for (y = y0; y <= last; y++)
    {
        ...
        LCD_BufferFill(buffer, a, y, b, y, POINT_COLOR);
    }
    sa = dx12 * (y - y1);
    sb = dx02 * (y - y0);
    for (; y <= y2; y++)
...
void LCD_BufferShowChar(uint16_t *buffer, uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t num, uint8_t size, uint8_t mode)
{
    uint8_t temp;                     // 临时变量，用于存储字符的字模数据
    uint8_t pos, t;                   // pos用于遍历字符的行，t用于遍历字符的列
    uint16_t colortemp = POINT_COLOR; // 保存当前的画笔颜色

    num = num - ' '; // 得到偏移后的值，将字符转换为字模数组中的索引
    // LCD_SetWindows(x, y, x + size / 2 - 1, y + size - 1); // 设置单个文字显示窗口，确定字符显示的区域
    if (!mode) // 非叠加方式
    {
        for (pos = 0; pos < size; pos++) // 遍历字符的每一行
        {
            if (size == 12)
                temp = asc2_1206[num][pos]; // 调用1206字体字模数据
            else
                temp = asc2_1608[num][pos]; // 调用1608字体字模数据
            for (t = 0; t < size / 2; t++)  // 遍历字符的每一列
            {
                uint16_t px = x + t;
                uint16_t py = y + pos;
                /*if (temp & 0x01) // 判断当前位是否为1
                    Lcd_WriteData_16Bit(fc); // 写入前景色
                else
                    Lcd_WriteData_16Bit(bc); // 写入背景色
                temp >>= 1; // 右移一位，处理下一个像素点*/
                (temp & 0x01) ? (buffer[py * lcddev.width + px] = fc) :
                                (buffer[py * lcddev.width + px] = bc);
                temp >>= 1;
            }
        }
    }
    else // 叠加方式
    {
        for (pos = 0; pos < size; pos++) // 遍历字符的每一行
        {
            ...
            for (t = 0; t < size / 2; t++)  // 遍历字符的每一列
            {
                POINT_COLOR = fc; // 设置画笔颜色为前景色
                if (temp & 0x01)  // 判断当前位是否为1
                {
                    // LCD_DrawPoint(x + t, y + pos); // 画一个点
                    uint16_t px = x + t;
                    uint16_t py = y + pos;
                    buffer[py * lcddev.width + px] = fc;
                }
                temp >>= 1; // 右移一位，处理下一个像素点
            }
        }
    }
    POINT_COLOR = colortemp; // 恢复原来的画笔颜色
    // LCD_SetWindows(0, 0, lcddev.width - 1, lcddev.height - 1); // 恢复窗口为全屏
}

void LCD_BufferShowNum(uint16_t *buffer, uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size)
{
    uint8_t t, temp;
    uint8_t enshow = 0;
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))
        {
            if (temp == 0)
            {
                LCD_BufferShowChar(buffer, x + (size / 2) * t, y, POINT_COLOR, BACK_COLOR, ' ', size, 0);
                continue;
            }
            else
            {
        ...
        LCD_BufferShowChar(buffer, x + (size / 2) * t, y, POINT_COLOR, BACK_COLOR, temp + '0', size, 0);
    }
}

void LCD_BufferShowString(uint16_t *buffer, uint16_t x, uint16_t y, uint8_t size, uint8_t *p, uint8_t mode)
{
    while ((*p <= '~') && (*p >= ' ')) // 判断是不是非法字符!
    {
        if (x > (lcddev.width - 1) || y > (lcddev.height - 1))
            return;
        LCD_BufferShowChar(buffer, x, y, POINT_COLOR, BACK_COLOR, *p, size, mode);
        x += size / 2;
        p++;
    }
}

void GUI_BufferDrawFont16(uint16_t *buffer, uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s, uint8_t mode)
{
    uint8_t i, j;
    uint16_t k;
    uint16_t HZnum;
            ...
            for (i = 0; i < 16 * 2; i++)
            {
                for (j = 0; j < 8; j++)
                {
                    if (!mode) // 非叠加方式
                    {
                        if (tfont16[k].Msk[i] & (0x80 >> j))
                        {
                            buffer[y * lcddev.width + x] = fc; // 写入前景色
                            // Lcd_WriteData_16Bit(fc);
                        }
                        else
                        {
                            buffer[y * lcddev.width + x] = bc; // 写入背景色
                            // Lcd_WriteData_16Bit(bc);
                        }
                    }
                    else
                    {
                        ...
                        if (tfont16[k].Msk[i] & (0x80 >> j))
                        {
                            LCD_BufferDrawPoint(buffer, x, y); // 画一个点
                        }
                        x++;
                        if ((x - x0) == 16)
                        {
